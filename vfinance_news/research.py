...
def research_with_gemini(content: str, focus_areas: list = None) -> str:
    """Compatibility alias used by tests and older callers."""
    result = research_with_minimax(content, focus_areas)
    result = result.replace("MiniMax research error", "Gemini research error")
    result = result.replace("MiniMax research timeout", "Gemini research timeout")
    result = result.replace("minimax-prompt not found", "Gemini CLI not found")
    return result
